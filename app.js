...
// --- globals ---
let onlineBots={},bannedBots=[],userStatus="Studying",userSeconds=0,myName="Amir";
...
function formatTime(s){s=Math.floor(s);return`${String(Math.floor(s/3600)).padStart(2,'0')}:${String(Math.floor((s%3600)/60)).padStart(2,'0')}:${String(s%60).padStart(2,'0')}`;}
function getEnergyColor(e){return e>60?"#2ecc71":e>30?"#f1c40f":"#e74c3c";}
function applyTheme(t){const m={magenta:"#ff007f",green:"#2ecc71",yellow:"#f1c40f",purple:"#9b59b6",orange:"#e67e22",red:"#e74c3c",white:"#f8fafc"};document.documentElement.style.setProperty('--theme-main',m[t]||"#00ffff");}
const ROOM_THEMES={default:{icon:'🏠',label:'Default',particle:null},library:{icon:'📚',label:'Library',particle:null},space:{icon:'🚀',label:'Space',particle:null},forest:{icon:'🌲',label:'Forest',particle:null}};
let roomTheme='default',roomParticleInterval=null;
function applyRoomTheme(t){
  roomTheme=t;  document.body.classList.remove('room-library','room-space','room-forest');
  if(t!=='default')document.body.classList.add('room-'+t);
  if(roomParticleInterval){clearInterval(roomParticleInterval);roomParticleInterval=null;}
}
...
function checkUrlRoom(){const r=new URLSearchParams(window.location.search).get('room');if(r)setTimeout(()=>{const name=prompt('Enter your name to join this room:');if(!name||!name.trim())return;document.getElementById('username').value=name.trim();const ji=document.getElementById('join-room-id');if(ji)ji.value=r;document.getElementById('btn-join-mode')?.click();document.getElementById('btn-study-friends')?.click();},400);}
...
async function restoreMultiSession(sess){
  ...
  document.getElementById('display-username').innerText=myName;
  document.getElementById('user-timer').innerText=formatTime(userSeconds);
  let pw=sess.roomPassword;
  if(!pw){try{var pwS=JSON.parse(localStorage.getItem('cozy_room_pw'))||{};}catch(e){var pwS={};}pw=pwS[sess.roomId]||'';}
  try{
    const meta=await getRoomMeta(sess.roomId);
    if(!meta)throw new Error('Room has expired');
    mpRoomMeta=meta;
    roomPassword=pw;
    await rejoinRoom(sess.roomId,myName,sess.isHost);
    if(sess.isHost){setIsHost(true);document.getElementById('btn-admin-panel').style.display='block';}
    document.getElementById('login-screen').classList.add('hidden');
    document.getElementById('app-screen').classList.remove('hidden');
    document.getElementById('chat-messages').innerHTML='';
    (sess.chatHistory||[]).forEach(msg=>restoreMsg(msg.text,msg.cls));
    updateShopDisplay();
    startMultiplayerTimers(sess.isHost,sess.roomId,pw);
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.getElementById('toggle-host-advanced').addEventListener('click',()=>document.getElementById('host-advanced-panel').classList.toggle('hidden'));
  document.getElementById('char-selection-mode').addEventListener('change',e=>{if(e.target.value==='custom'){document.getElementById('manual-chars-wrapper').classList.remove('hidden');renderCharacterCheckboxes();}else document.getElementById('manual-chars-wrapper').classList.add('hidden');});
  document.getElementById('btn-toggle-creator').addEventListener('click',()=>document.getElementById('character-creator-panel').classList.toggle('hidden'));
  document.getElementById('allow-leave').addEventListener('change',e=>document.getElementById('rejoin-wrapper').classList.toggle('hidden',!e.target.checked));
  document.getElementById('btn-clear-history').addEventListener('click',()=>{if(confirm("Wipe all presets?")){localStorage.removeItem('cozy_presets');loadPresetsFromStorage();}});
  ...
  document.getElementById('btn-enter').addEventListener('click',()=>{
    const name=document.getElementById('username').value.trim();if(!name){alert("Enter your name!");return;}
    ...
    if(configSelectionMode==='custom'){configSelectedPool=[];document.querySelectorAll('.bot-selector-check:checked').forEach(c=>configSelectedPool.push(c.value));if(!configSelectedPool.length){alert("Pick at least one!");return;}}
    else configSelectedPool=getAllCharacters().map(c=>c.name);
    const presetName=document.getElementById('new-preset-name').value.trim();
    if(presetName){try{var s=JSON.parse(localStorage.getItem('cozy_presets'))||{};}catch(e){var s={};}s[presetName]={user:name,theme,max:configMaxMembers,mode:configSelectionMode,selectedPool:configSelectedPool,allowOthers:configAllowOthers,allowLeave:configAllowLeave,allowRejoin:configAllowRejoin};localStorage.setItem('cozy_presets',JSON.stringify(s));}
    myName=name;userSeconds=0;userCoins=0;coffeeStock=0;onlineBots={};bannedBots=[];
    applyTheme(theme);
    ...
    document.getElementById('btn-admin-panel').style.display='block';
    initRoom(false);
    saveSession({mode:'solo',userName:name,theme,userSeconds:0,userCoins:0,coffeeStock:0,configMaxMembers,configSelectionMode,configSelectedPool,configAllowOthers,configAllowLeave,configAllowRejoin,configAllowBotChat,bots:sanitizeBotsForStorage()});
  });
  ...
  document.getElementById('btn-pause').addEventListener('click',()=>{
    ...
    if(s){btn.textContent='⏸ BREAK';btn.style.background='#2a1a1a';btn.style.borderColor='#e67e22';btn.style.color='#e67e22';}
    else{btn.textContent='▶ CONTINUE';btn.style.background='#1a2a1a';btn.style.borderColor='#2ecc71';btn.style.color='#2ecc71';}
  ...
  document.getElementById('btn-shop').addEventListener('click',()=>{updateShopDisplay();document.getElementById('shop-modal').classList.remove('hidden');});
  ...
  document.getElementById('btn-buy-coffee').addEventListener('click',()=>{
    if(userCoins<COFFEE_PRICE){addMsg(`💰 Need ${COFFEE_PRICE} coins, have ${Math.floor(userCoins)}.`,'system-msg');return;}
    userCoins-=COFFEE_PRICE;coffeeStock++;updateShopDisplay();addMsg(`☕ Bought coffee! Stock: ${coffeeStock}`,'system-msg');
  });
  ...
  // friends modal
  document.getElementById('btn-study-friends').addEventListener('click',async()=>{if(!document.getElementById('username').value.trim()){alert("Enter your name first!");return;}const btn=document.getElementById('btn-study-friends');const orig=btn.innerHTML;btn.innerHTML+=' <span class="btn-spinner"></span>';btn.classList.add('loading-btn');await loadActiveRooms();btn.innerHTML=orig;btn.classList.remove('loading-btn');document.getElementById('friends-modal').classList.remove('hidden');});
  document.getElementById('btn-close-friends').addEventListener('click',()=>document.getElementById('friends-modal').classList.add('hidden'));
  document.getElementById('btn-host-mode').addEventListener('click',()=>{document.getElementById('host-panel').classList.remove('hidden');document.getElementById('join-panel').classList.add('hidden');});
  document.getElementById('btn-join-mode').addEventListener('click',()=>{document.getElementById('join-panel').classList.remove('hidden');document.getElementById('host-panel').classList.add('hidden');});
  document.getElementById('btn-create-room').addEventListener('click',createRoom);
  document.getElementById('btn-copy-link').addEventListener('click',()=>navigator.clipboard.writeText(document.getElementById('room-link-display').innerText).then(()=>{document.getElementById('btn-copy-link').innerText='✅ COPIED!';setTimeout(()=>document.getElementById('btn-copy-link').innerText='📋 COPY LINK',2000);}));
  ...
  const statsBtn=document.createElement('button');statsBtn.id='btn-stats';statsBtn.className='pixel-btn';statsBtn.style.cssText='width:auto;padding:2px 8px;font-size:9px;margin:0;height:22px;background:#1a2a2a;border-color:#00ffff;color:#00ffff;';statsBtn.textContent='📊 STATS';
  const powerBtn=document.getElementById('btn-power');powerBtn.parentNode.insertBefore(statsBtn,powerBtn);
  statsBtn.addEventListener('click',showStats);

  // Add Join Desk button between name and status badge
  const deskBtn=document.createElement('button');deskBtn.id='btn-join-desk';deskBtn.className='desk-join-btn';deskBtn.textContent='🪑 JOIN DESK';
  ...
  if(loginBox){
    ...
    const pickerBtn=document.createElement('button');pickerBtn.type='button';pickerBtn.className='theme-picker-icon';pickerBtn.textContent='🎨';pickerBtn.title='Room Theme';
    loginBox.appendChild(pickerBtn);
    pickerBtn.addEventListener('click',()=>{
      const popup=document.createElement('div');popup.className='theme-popup';
      let cardsHTML='';Object.entries(ROOM_THEMES).forEach(([key,val])=>{
  ...
  setInterval(()=>{for(let n in onlineBots){const b=onlineBots[n];if(b.coffeeWaiting&&b.coffeeMessageId){const r=Math.max(0,COFFEE_WAIT_SECONDS-b.coffeeWaitSeconds);const el=document.getElementById('coffee-timer-'+b.coffeeMessageId);if(el)el.innerText=`${Math.floor(r/60)}:${String(r%60).padStart(2,'0')} remaining`;}}},1000);
  ...
  document.addEventListener('keydown',e=>{
    const appVisible=document.getElementById('app-screen')&&!document.getElementById('app-screen').classList.contains('hidden');
    if(!appVisible)return;
    const anyModalOpen=!document.getElementById('shop-modal').classList.contains('hidden')||!document.getElementById('admin-modal').classList.contains('hidden')||!document.getElementById('friends-modal').classList.contains('hidden')||!document.getElementById('stats-modal')?.classList.contains('hidden');
    if(e.key==='Escape'){
      document.getElementById('shop-modal').classList.add('hidden');
      document.getElementById('admin-modal').classList.add('hidden');
      document.getElementById('friends-modal').classList.add('hidden');
      const statsModal=document.getElementById('stats-modal');if(statsModal)statsModal.remove();
      return;
    }
...
// --- Study Stats ---
let totalStudySessions=0,totalCharactersHelped=0;
function loadStats(){try{const s=JSON.parse(localStorage.getItem('cozy_stats'));if(s){totalStudySessions=s.sessions||0,totalCharactersHelped=s.helped||0;}}catch(e){}}
function incrementStat(type){if(type==='session')totalStudySessions++;if(type==='helped')totalCharactersHelped++;localStorage.setItem('cozy_stats',JSON.stringify({sessions:totalStudySessions,helped:totalCharactersHelped}));}
function showStats(){
  const s=document.createElement('div');s.id='stats-modal';s.className='screen hidden';s.style.cssText='position:fixed;top:0;left:0;background:rgba(0,0,0,0.85);z-index:999;';
...
function joinDesk(){
  ...
  document.getElementById('desk-todo-input').addEventListener('keydown',e=>{if(e.key==='Enter')addDeskTodo();});
  document.getElementById('desk-chat-send').addEventListener('click',sendDeskChat);
  document.getElementById('desk-chat-input').addEventListener('keydown',e=>{if(e.key==='Enter')sendDeskChat();});
  ...
  deskPomoVisualInterval=setInterval(()=>{
    updateDeskTimer();
    updateDeskMemberCount();
  },1000);
}
...
function leaveDesk(){
  deskActive=false;
  if(deskPomoVisualInterval){clearInterval(deskPomoVisualInterval);deskPomoVisualInterval=null;}
  deskPomoVisualMode=null;deskPomoVisualTime=0;
  const cd=document.getElementById('desk-pomo-countdown');if(cd)cd.innerHTML='';
  const d=document.getElementById('desk-screen');if(d)d.remove();
}

function renderDeskTodos(){
  const list=document.getElementById('desk-todo-list');if(!list)return;
  list.innerHTML='';
  deskTodos.forEach((todo,i)=>{
    const item=document.createElement('div');item.className='desk-todo-item'+(todo.done?' done':'');
    ...
    item.querySelector('.desk-todo-check').addEventListener('click',()=>{deskTodos[i].done=!deskTodos[i].done;saveDeskTodos();renderDeskTodos();});
    item.querySelector('.desk-todo-delete').addEventListener('click',()=>{deskTodos.splice(i,1);saveDeskTodos();renderDeskTodos();});
    item.addEventListener('dragstart',e=>{e.dataTransfer.setData('text/plain',i);item.classList.add('dragging');});
    item.addEventListener('dragend',()=>item.classList.remove('dragging'));
    item.addEventListener('dragover',e=>{e.preventDefault();item.style.borderColor='var(--theme-main)';});
    item.addEventListener('dragleave',()=>{item.style.borderColor='';});
    item.addEventListener('drop',e=>{e.preventDefault();item.style.borderColor='';const from=parseInt(e.dataTransfer.getData('text/plain'));const to=i;if(from!==to){const moved=deskTodos.splice(from,1)[0];const adjustedTo=from<to?to-1:to;deskTodos.splice(adjustedTo,0,moved);saveDeskTodos();renderDeskTodos();}});
    list.appendChild(item);
  });
}

function addDeskTodo(){
  const input=document.getElementById('desk-todo-input');if(!input)return;
  const text=input.value.trim();if(!text||deskTodos.length>=20)return;
  deskTodos.push({text,done:false});saveDeskTodos();renderDeskTodos();input.value='';
}

function renderDeskChat(){
  const c=document.getElementById('desk-chat-messages');if(!c)return;
  c.innerHTML='';
  chatHistory.filter(msg=>msg.cls!=='system-msg').forEach(msg=>{const m=document.createElement('div');m.className='message '+msg.cls;m.innerHTML=msg.text;c.appendChild(m);});
...
function updateDeskTimer(){
  const el=document.getElementById('desk-timer');if(!el)return;
  el.innerText=formatTime(userSeconds);
}

function updateDeskMemberCount(){
  const el=document.getElementById('desk-member-count');if(!el)return;
  const count=Object.keys(onlineBots).length+1;
  el.innerText=`👥 ${count} online`;
}

function toggleDeskPomodoro(){
  const btn=document.getElementById('desk-pomo-btn');if(!btn)return;
  if(deskPomoVisualMode==='pomo'){
    deskPomoVisualMode=null;deskPomoVisualTime=0;
    if(deskPomoVisualInterval){clearInterval(deskPomoVisualInterval);deskPomoVisualInterval=null;}
    deskPomoVisualInterval=setInterval(()=>{updateDeskTimer();updateDeskMemberCount();},1000);
    btn.classList.remove('active');btn.innerText='🍅 Pomodoro';
    document.getElementById('desk-timer-label').innerText='Study Time';
    document.getElementById('desk-pomo-countdown').innerHTML='';
    if(userStatus==='Relaxing'){userStatus='Studying';document.getElementById('user-status-badge').innerText='Studying 📖';document.getElementById('user-status-badge').className='status-badge studying';document.getElementById('btn-pause').textContent='⏸ BREAK';document.getElementById('btn-pause').style.background='#2a1a1a';document.getElementById('btn-pause').style.borderColor='#e67e22';document.getElementById('btn-pause').style.color='#e67e22';addMsg('📖 You resumed studying!','system-msg');}
    updateMemberList();
  }else{
    if(deskPomoVisualMode==='break'){clearInterval(deskPomoVisualInterval);deskPomoVisualInterval=null;}
    deskPomoVisualMode='pomo';deskPomoVisualTime=25*60;
    btn.classList.add('active');btn.innerText='⏸ Stop';
    document.getElementById('desk-timer-label').innerText='🍅 Pomodoro Focus';
    document.getElementById('desk-break-btn').classList.remove('active');document.getElementById('desk-break-btn').innerText='☕ Break';
    if(userStatus!=='Studying'){userStatus='Studying';document.getElementById('user-status-badge').innerText='Studying 📖';document.getElementById('user-status-badge').className='status-badge studying';document.getElementById('btn-pause').textContent='⏸ BREAK';document.getElementById('btn-pause').style.background='#2a1a1a';document.getElementById('btn-pause').style.borderColor='#e67e22';document.getElementById('btn-pause').style.color='#e67e22';}
    addMsg('🍅 Pomodoro started! Focus for 25 minutes.','system-msg');updateMemberList();
    startDeskPomoVisual();
  }
}

function startDeskBreak(){
  if(deskPomoVisualMode==='break'){
    deskPomoVisualMode=null;deskPomoVisualTime=0;
    if(deskPomoVisualInterval){clearInterval(deskPomoVisualInterval);deskPomoVisualInterval=null;}
    deskPomoVisualInterval=setInterval(()=>{updateDeskTimer();updateDeskMemberCount();},1000);
    document.getElementById('desk-break-btn').classList.remove('active');document.getElementById('desk-break-btn').innerText='☕ Break';
  ...
  if(deskPomoVisualMode==='pomo'){clearInterval(deskPomoVisualInterval);deskPomoVisualInterval=null;document.getElementById('desk-pomo-btn').classList.remove('active');document.getElementById('desk-pomo-btn').innerText='🍅 Pomodoro';}
  deskPomoVisualMode='break';deskPomoVisualTime=5*60;
  const btn=document.getElementById('desk-break-btn');btn.classList.add('active');btn.innerText='⏸ Stop';
  document.getElementById('desk-timer-label').innerText='☕ Break Time';
  if(userStatus!=='Relaxing'){userStatus='Relaxing';document.getElementById('user-status-badge').innerText='Relaxing ⏳';document.getElementById('user-status-badge').className='status-badge relaxing';document.getElementById('btn-pause').textContent='▶ CONTINUE';document.getElementById('btn-pause').style.background='#1a2a1a';document.getElementById('btn-pause').style.borderColor='#2ecc71';document.getElementById('btn-pause').style.color='#2ecc71';}
  addMsg('☕ Break time! Relax for 5 minutes.','system-msg');updateMemberList();
  startDeskPomoVisual();
}

function startDeskPomoVisual(){
  if(deskPomoVisualInterval)clearInterval(deskPomoVisualInterval);
  updatePomoVisualDisplay();
  deskPomoVisualInterval=setInterval(()=>{
    if(deskPomoVisualTime>0){deskPomoVisualTime--;updatePomoVisualDisplay();}
    else{
      clearInterval(deskPomoVisualInterval);deskPomoVisualInterval=null;
      const cd=document.getElementById('desk-pomo-countdown');
      if(deskPomoVisualMode==='pomo'){
        alert('🍅 Pomodoro done! Time for a break.');addMsg('🎯 Pomodoro finished!','system-msg');
        document.getElementById('desk-pomo-btn').classList.remove('active');document.getElementById('desk-pomo-btn').innerText='🍅 Pomodoro';
        document.getElementById('desk-timer-label').innerText='Study Time';if(cd)cd.innerHTML='';
        deskPomoVisualMode=null;
        userStatus='Studying';document.getElementById('user-status-badge').innerText='Studying 📖';document.getElementById('user-status-badge').className='status-badge studying';
        document.getElementById('btn-pause').textContent='⏸ BREAK';document.getElementById('btn-pause').style.background='#2a1a1a';document.getElementById('btn-pause').style.borderColor='#e67e22';document.getElementById('btn-pause').style.color='#e67e22';
        deskPomoVisualInterval=setInterval(()=>{updateDeskTimer();updateDeskMemberCount();},1000);
      }else{
        alert('☕ Break over! Back to studying.');addMsg('📖 Break over! Back to studying.','system-msg');
        document.getElementById('desk-break-btn').classList.remove('active');document.getElementById('desk-break-btn').innerText='☕ Break';
        document.getElementById('desk-timer-label').innerText='Study Time';if(cd)cd.innerHTML='';
        deskPomoVisualMode=null;
        userStatus='Studying';document.getElementById('user-status-badge').innerText='Studying 📖';document.getElementById('user-status-badge').className='status-badge studying';
...
function updatePomoVisualDisplay(){
  const cd=document.getElementById('desk-pomo-countdown');if(!cd)return;
  if(!deskPomoVisualMode){cd.innerHTML='';return;}
  const color=deskPomoVisualMode==='pomo'?'#e74c3c':'#2ecc71';
  const label=deskPomoVisualMode==='pomo'?'🍅 Pomodoro':'☕ Break';
  cd.innerHTML=`<div style="font-size:20px;font-weight:bold;color:${color};margin-top:8px;">${label}: ${formatTime(deskPomoVisualTime)}</div>`;
}

// --- reset UI ---
function resetUI(){
  onlineBots={};bannedBots=[];userStatus='Studying';userSeconds=0;userCoins=0;coffeeStock=0;lastSeenMessages={};seenMpCoffeeBots=new Set();lastMpMembers=null;lastMpMyUserName=null;chatHistory=[];
  if(roomParticleInterval){clearInterval(roomParticleInterval);roomParticleInterval=null;}
  ...
  document.getElementById('chat-messages').innerHTML='<div class="message system-msg">ℹ️ Room initialized. Welcome!</div>';
...
function initRoom(skipGenerate){
  ...
  timerInterval=setInterval(()=>{
    if(userStatus==='Studying'){userSeconds++;userCoins+=COINS_PER_MINUTE/60;document.getElementById('user-timer').innerText=formatTime(userSeconds);}
    tickBots();
    if(userSeconds%10===0){
      updateMemberList();
...
function sanitizeBotsForStorage(){const out={};for(let n in onlineBots){const b=onlineBots[n];out[n]={status:b.status,timeSpent:b.timeSpent,energy:b.energy,energyTickStudy:b.energyTickStudy||0,energyTickRelax:b.energyTickRelax||0,coffeeWaiting:false,coffeeWaitSeconds:0,coffeeMessageId:null,pass:b.pass,isFrozen:b.isFrozen,isLocked:b.isLocked,color:b.color,quotes:b.quotes};}return out;}

function tickBots(){
  for(let bot in onlineBots){
    const b=onlineBots[bot];if(b.isFrozen)continue;
    if(b.status==='Studying'&&!b.coffeeWaiting){b.timeSpent+=1/60;b.energyTickStudy=(b.energyTickStudy||0)+1;if(b.energyTickStudy>=ENERGY_DRAIN_INTERVAL){b.energy=Math.max(0,b.energy-1);b.energyTickStudy=0;if(b.energy===0)triggerCoffeeRequest(bot);}}
    else if(b.status==='Relaxing'&&!b.coffeeWaiting){b.energyTickRelax=(b.energyTickRelax||0)+1;if(b.energyTickRelax>=ENERGY_RECOVER_INTERVAL){b.energy=Math.min(100,b.energy+1);b.energyTickRelax=0;if(b.energy>=30)b.status='Studying';}}
    if(b.coffeeWaiting){b.coffeeWaitSeconds++;if(b.coffeeWaitSeconds>=COFFEE_WAIT_SECONDS)expireCoffeeRequest(bot);}
  }
}

function triggerCoffeeRequest(name){
  const b=onlineBots[name];if(!b||b.coffeeWaiting)return;
  b.coffeeWaiting=true;b.coffeeWaitSeconds=0;b.status='Relaxing';
  const msgId='coffee-'+name.replace(/[^a-zA-Z0-9]/g,'-')+'-'+Date.now();b.coffeeMessageId=msgId;
  addMsg(`😩 <b>${sanitizeHTML(name)}:</b> I'm completely drained... I need coffee!<br><span class="coffee-btn waiting" data-coffee-name="${sanitizeHTML(name)}" title="Give Coffee">☕</span> <span style="font-size:9px;color:#665b75;" id="coffee-timer-${msgId}">2:00 remaining</span>`,'bot-msg-relax');
...
function expireCoffeeRequest(name){
  const b=onlineBots[name];if(!b)return;
  if(b.coffeeMessageId){const el=document.getElementById(b.coffeeMessageId);if(el){const btn=el.querySelector('.coffee-btn');if(btn){btn.classList.remove('waiting');btn.classList.add('expired');btn.onclick=null;}const te=document.getElementById('coffee-timer-'+b.coffeeMessageId);if(te)te.innerText='⏰ Too late!';}}
  if(!configAllowLeave){
    addMsg(`☕ <b>${name}</b> missed coffee but stays (leave disabled).`,'system-msg');
    b.coffeeWaiting=false;b.coffeeWaitSeconds=0;b.coffeeMessageId=null;b.status='Studying';b.energy=Math.max(10,b.energy+20);
  }else{
...
window.giveCoffee=function(name){
  ...
  if(coffeeStock<=0){addMsg('☕ No coffee! Buy from 🛒 SHOP.','system-msg');return;}
  coffeeStock--;onlineBots[name].energy=Math.min(100,onlineBots[name].energy+70);
  onlineBots[name].coffeeWaiting=false;onlineBots[name].coffeeWaitSeconds=0;onlineBots[name].status='Studying';
  const mid=onlineBots[name].coffeeMessageId;
  if(mid){const el=document.getElementById(mid);if(el){const btn=el.querySelector('.coffee-btn');if(btn){btn.classList.remove('waiting');btn.onclick=null;btn.style.cursor='default';}const te=document.getElementById('coffee-timer-'+mid);if(te)te.innerText='✅ Delivered!';}}
  ...
  updateShopDisplay();updateMemberList();
};
...
function startSimulation(){
  ...
  simInterval=setInterval(()=>{
    ...
    const n=botNames[Math.floor(Math.random()*botNames.length)];const b=onlineBots[n];
...
function updateMemberList(){
  const ml=document.getElementById('member-list');ml.innerHTML='';
  if(typeof isInMultiplayer==='function'&&isInMultiplayer())return;
  document.getElementById('member-count').innerText=`👥 Active Leaders (${Object.keys(onlineBots).length+1}/${configMaxMembers})`;
  let list=Object.keys(onlineBots).map(n=>({name:n,...onlineBots[n],isUser:false}));
  list.push({name:myName+' (YOU)',timeSpent:userSeconds/60,status:userStatus,isUser:true,isFrozen:false,isLocked:false,energy:100,coffeeWaiting:false,color:'var(--theme-main)'});
  ...
  list.forEach(m=>{
    ...
    const icon=m.coffeeWaiting?'☕':(m.isLocked?'🔒':(m.isFrozen?'❄️':(m.status==='Studying'?'📖':'⏳')));
    const eBar=m.isUser?'':`<div class="energy-bar-wrap"><div class="energy-bar-fill" style="width:${m.energy}%;background:${getEnergyColor(m.energy)};"></div></div>`;
...
function updateMemberTimesOnly(){
  for(let n in onlineBots){const el=document.getElementById('time-'+n.replace(/[^a-zA-Z0-9]/g,'-'));if(el&&onlineBots[n].status==='Studying'&&!onlineBots[n].isFrozen)el.innerText=formatTime(Math.floor(onlineBots[n].timeSpent*60));}
  const myEl=document.getElementById('time-'+(myName+' (YOU)').replace(/[^a-zA-Z0-9]/g,'-'));if(myEl&&userStatus==='Studying')myEl.innerText=formatTime(userSeconds);
}

// --- ADMIN ---
function renderAdminPanelUsers(){
  const list=document.getElementById('admin-users-list');list.innerHTML='';
  for(let name in onlineBots){const b=onlineBots[name];const row=document.createElement('div');row.className='admin-row';
    const credentials=document.createElement('div');credentials.className='credentials';
    ...
    const lockBtn=document.createElement('button');lockBtn.className=b.isLocked?'admin-btn unlock':'admin-btn lock';lockBtn.textContent=b.isLocked?'UNLOCK':'LOCK';lockBtn.addEventListener('click',()=>adminToggleLock(name));
    const freezeBtn=document.createElement('button');freezeBtn.className=b.isFrozen?'admin-btn unfreeze':'admin-btn freeze';freezeBtn.textContent=b.isFrozen?'UNFREEZE':'FREEZE';freezeBtn.addEventListener('click',()=>adminToggleFreeze(name));
    const resetBtn=document.createElement('button');resetBtn.className='admin-btn reset';resetBtn.textContent='RESET';resetBtn.addEventListener('click',()=>adminResetTime(name));
    const banBtn=document.createElement('button');banBtn.className='admin-btn ban';banBtn.textContent='BAN';banBtn.addEventListener('click',()=>adminBan(name));
    actions.append(lockBtn,freezeBtn,resetBtn,banBtn);
    row.append(credentials,actions);list.appendChild(row);}
}
window.adminToggleFreeze=function(n){if(onlineBots[n]){onlineBots[n].isFrozen=!onlineBots[n].isFrozen;addMsg(`${onlineBots[n].isFrozen?'❄️':'🔥'} ${n}`,'system-msg');updateMemberList();renderAdminPanelUsers();}};
window.adminToggleLock=function(n){if(onlineBots[n]){onlineBots[n].isLocked=!onlineBots[n].isLocked;addMsg(`${onlineBots[n].isLocked?'🔒':'🔓'} ${n}`,'system-msg');updateMemberList();renderAdminPanelUsers();}};
window.adminResetTime=function(n){if(onlineBots[n]){onlineBots[n].timeSpent=0;addMsg(`🧹 ${n} time reset.`,'system-msg');updateMemberList();renderAdminPanelUsers();}};
window.adminBan=function(n){if(onlineBots[n]){bannedBots.push(n);delete onlineBots[n];addMsg(`🚫 ${n} banned.`,'bot-msg-leave');updateMemberList();renderAdminPanelUsers();}};

// --- WEATHER ---
function changeWeather(type){
  clearInterval(weatherInterval);const ov=document.getElementById('weather-overlay');ov.innerHTML='';
  document.body.className=document.body.className.replace(/weather-\w+/g,'');
  if(type==='clear'){document.body.classList.add('weather-clear');}
  else if(type==='rainy'){document.body.classList.add('weather-rainy');weatherInterval=setInterval(()=>{for(let i=0;i<3;i++){const d=document.createElement('div');d.className='drop';d.style.left=Math.random()*100+'vw';d.style.animationDuration=(Math.random()*0.5+0.5)+'s';d.style.opacity=Math.random()*0.4+0.2;ov.appendChild(d);setTimeout(()=>d.remove(),1000);}},60);}
  else if(type==='snowy'){document.body.classList.add('weather-snowy');weatherInterval=setInterval(()=>{for(let i=0;i<2;i++){const f=document.createElement('div');f.className='flake';f.style.left=Math.random()*100+'vw';f.style.animationDuration=(Math.random()*2+2)+'s';f.style.width=(Math.random()*3+2)+'px';f.style.height=f.style.width;f.style.opacity=Math.random()*0.6+0.3;ov.appendChild(f);setTimeout(()=>f.remove(),4000);}},150);}
  updateWeatherBtnStyle(type);
}
function updateWeatherBtnStyle(type){
  const el=document.getElementById('weather-select');if(!el)return;
  const styles={clear:{bg:'#2a2a1a',border:'#f1c40f'},rainy:{bg:'#1a1a2a',border:'#3498db'},snowy:{bg:'#1a2a2a',border:'#a29bfe'}};
  const s=styles[type]||styles.clear;
  el.style.background=s.bg;el.style.borderColor=s.border;el.style.color=s.border;
}

// --- POMODORO ---
function initPomodoro(){let tl=25*60,run=false,t=null;const pm=document.getElementById('pomo-minutes'),ps=document.getElementById('pomo-seconds'),bs=document.getElementById('btn-start-pomo'),br=document.getElementById('btn-reset-pomo');function upd(){pm.innerText=String(Math.floor(tl/60)).padStart(2,'0');ps.innerText=String(tl%60).padStart(2,'0');}bs.addEventListener('click',()=>{if(run){clearInterval(t);bs.innerText='START';bs.style.background='var(--theme-main)';bs.style.color='#000';run=false;}else{run=true;bs.innerText='PAUSE';bs.style.background='#e67e22';bs.style.color='#fff';t=setInterval(()=>{if(tl>0){tl--;upd();}else{clearInterval(t);run=false;tl=25*60;bs.innerText='START';bs.style.background='var(--theme-main)';bs.style.color='#000';upd();alert('🚨 Pomodoro done!');addMsg('🎯 Pomodoro finished!','system-msg');}},1000);}});br.addEventListener('click',()=>{clearInterval(t);run=false;tl=25*60;bs.innerText='START';bs.style.background='var(--theme-main)';bs.style.color='#000';upd();});}

// --- PRESETS ---
function loadPresetsFromStorage(){try{var s=JSON.parse(localStorage.getItem('cozy_presets'))||{};}catch(e){var s={};}const sel=document.getElementById('preset-history');sel.innerHTML='<option value="default">✨ Default Cozy Room</option>';for(let n in s){const o=document.createElement('option');o.value=n;o.innerText=`⏳ [History] ${n}`;sel.appendChild(o);}}
function applyPreset(e){if(e.target.value==='default')return;try{var presets=JSON.parse(localStorage.getItem('cozy_presets'))||{};}catch(e){var presets={};}const p=presets[e.target.value];if(!p)return;document.getElementById('username').value=p.user||'';document.getElementById('theme-color').value=p.theme||'cyan';document.getElementById('max-members').value=p.max;document.getElementById('char-selection-mode').value=p.mode;document.getElementById('allow-others').checked=p.allowOthers;document.getElementById('allow-leave').checked=p.allowLeave;document.getElementById('allow-rejoin').checked=p.allowRejoin;if(p.mode==='custom'){document.getElementById('manual-chars-wrapper').classList.remove('hidden');renderCharacterCheckboxes();document.querySelectorAll('.bot-selector-check').forEach(c=>{c.checked=p.selectedPool.includes(c.value);});}document.getElementById('advanced-panel').classList.remove('hidden');}
function renderCharacterCheckboxes(){const l=document.getElementById('chars-checkbox-list');l.innerHTML='';getAllCharacters().forEach(c=>{const r=document.createElement('div');r.className='checkbox-item-row';const label=document.createElement('label');label.className='pixel-checkbox';const cb=document.createElement('input');cb.type='checkbox';cb.className='bot-selector-check';cb.value=c.name;cb.checked=true;const span=document.createElement('span');span.className='checkmark';label.append(cb,span,' '+c.name);r.appendChild(label);if(c.isCustom){const delBtn=document.createElement('button');delBtn.type='button';delBtn.className='btn-delete-char';delBtn.textContent='❌';delBtn.addEventListener('click',()=>deleteCustomCharacter(c.name));r.appendChild(delBtn);}l.appendChild(r);});}
window.deleteCustomCharacter=function(name){if(confirm(`Delete "${name}"?`)){try{var p=JSON.parse(localStorage.getItem('cozy_custom_chars'))||[];}catch(e){var p=[];}localStorage.setItem('cozy_custom_chars',JSON.stringify(p.filter(c=>c.name!==name)));renderCharacterCheckboxes();}};
function saveCustomChar(){const name=document.getElementById('new-char-name').value.trim();if(!name){alert('Enter a name!');return;}const pass=document.getElementById('new-char-pass').value.trim()||'pass123';const sq=document.getElementById('new-char-study').value.trim();const rq=document.getElementById('new-char-relax').value.trim();const obj={name,pass,studyQuotes:sq?sq.split('|').map(q=>q.trim()):["Let's study! 📖"],relaxQuotes:rq?rq.split('|').map(q=>q.trim()):['Taking a break ☕'],isCustom:true};try{var p=JSON.parse(localStorage.getItem('cozy_custom_chars'))||[];}catch(e){var p=[];}p=p.filter(c=>c.name.toLowerCase()!==name.toLowerCase());p.push(obj);localStorage.setItem('cozy_custom_chars',JSON.stringify(p));['new-char-name','new-char-pass','new-char-study','new-char-relax'].forEach(id=>document.getElementById(id).value='');document.getElementById('character-creator-panel').classList.add('hidden');renderCharacterCheckboxes();}

// --- MULTIPLAYER ---
async function loadActiveRooms(){
  const container=document.getElementById('active-rooms-list');container.innerHTML='';
  try{var saved=JSON.parse(localStorage.getItem('cozy_my_rooms'))||[];}catch(e){var saved=[];}if(!saved.length)return;
  const active=await getActiveRooms(saved);if(!active.length)return;
  container.innerHTML='<div style="font-size:9px;color:#8e81a0;margin-bottom:6px;">🟢 YOUR ACTIVE ROOMS:</div>';
  active.forEach(r=>{const div=document.createElement('div');div.style.cssText='background:#0d1a0d;border:1px solid #2ecc71;padding:8px 12px;border-radius:4px;margin-bottom:6px;display:flex;justify-content:space-between;align-items:center;cursor:pointer;font-size:10px;';div.innerHTML=`<span style="color:#2ecc71;">🏠 <b>${r.id}</b> — <b>${r.memberCount}</b> online</span><span style="color:#8e81a0;font-size:9px;">host: ${r.host}</span>`;div.onclick=()=>{document.getElementById('join-room-id').value=r.id;document.getElementById('btn-join-mode').click();};container.appendChild(div);});
}
...
async function createRoom(){
  ...
  const st=document.getElementById('friends-status-msg');st.style.color='#f1c40f';st.innerText='Creating room...';
  try{
    const roomId=await hostRoom(name,pw,maxM,allowBots);
    try{var mr=JSON.parse(localStorage.getItem('cozy_my_rooms'))||[];}catch(e){var mr=[];}if(!mr.includes(roomId))mr.push(roomId);localStorage.setItem('cozy_my_rooms',JSON.stringify(mr));
    ...
    try{var pwS=JSON.parse(localStorage.getItem('cozy_room_pw'))||{};}catch(e){var pwS={};}pwS[roomId]=pw;localStorage.setItem('cozy_room_pw',JSON.stringify(pwS));
    st.style.color='#2ecc71';st.innerText='✅ Room created!';
    mpRoomMeta={maxMembers:maxM,allowBots};
  }catch(e){st.style.color='#e74c3c';st.innerText='Error: '+e.message;}
}
...
async function joinRoomHandler(){
  ...
  const name=document.getElementById('username').value.trim();
  if(!input||!pw){alert('Fill Room ID and password!');return;}
  if(input.includes('?room='))input=input.split('?room=')[1];
  const roomId=input.toUpperCase();
  const st=document.getElementById('friends-status-msg');st.style.color='#f1c40f';st.innerText='Joining room...';
  try{const meta=await joinRoom(roomId,pw,name);mpRoomMeta=meta;st.style.color='#2ecc71';st.innerText='✅ Joined! Entering...';setTimeout(()=>enterMultiplayerRoom(false,roomId,pw),800);}
  catch(e){st.style.color='#e74c3c';st.innerText='Error: '+e.message;}
}
...
function enterMultiplayerRoom(asHost,roomId,pw){
  ...
  roomPassword=rpw;
  try{var saved=JSON.parse(localStorage.getItem('cozy_mp_session'))||{};}catch(e){var saved={};}delete saved[rid];localStorage.setItem('cozy_mp_session',JSON.stringify(saved));
...
function setupBotsForMultiplayer(meta){
  ...
  configAllowOthers=true;configAllowLeave=true;configAllowRejoin=true;
...
function renderMpBots(ml){
  ...
  Object.entries(bots).forEach(([n,b])=>{
    const card=document.createElement('div');card.className='member-card bot-mp-card';
...
function renderMpMemberList(members,myUserName){
  ...
  Object.entries(members).forEach(([name,data])=>{
    const card=document.createElement('div');card.className='member-card';
    const isMe=name===myUserName;const icon=data.status==='Studying'?'📖':'⏳';const color=isMe?'var(--theme-main)':'#a29bfe';
    const timeDisplay=isMe?`<span class="m-time" id="mp-time-me">${formatTime(userSeconds)}</span>`:`<span class="m-time">${formatTime(data.timeSpent||0)}</span>`;
...
window.onMultiplayerMembersUpdate=function(members,myUserName){
  lastMpMembers=members;lastMpMyUserName=myUserName;
    ...
    if(b.coffeeWaiting){
      if(!seenMpCoffeeBots.has(n)){
        ...
        if(!document.getElementById(mid)){
          addMsg(`😩 <b>${sanitizeHTML(n)}:</b> I need coffee!<br><span class="coffee-btn waiting" data-coffee-mp="${sanitizeHTML(n)}" title="Give Coffee">☕</span>`,'bot-msg-relax');
          const c=document.getElementById('chat-messages');
          if(c&&c.lastChild){c.lastChild.id=mid;c.lastChild.querySelector('.coffee-btn').addEventListener('click',()=>giveCoffeeMp(n));}
        }
      }
    }else{
